  ...
  return function (program) {
    program
      ...
      .action(function (selector, cmd) {
        ...
        // ==== Allocation settings ====
        // global variables
        target_prop = [.106, .6000, .20000]
        var cash_target = 0.15
        setTargets()
        
        // Triggers, updates the cash_target based on value
        function checkTriggers(val, params=null) {
          
          if (val > 39000 && val < 41500 && cash_target != 0.155) {
            cash_target = 0.155
            setTargets()
          } else if (val > 42000 && val < 43500 && cash_target != 0.16) {
            cash_target = 0.16
            setTargets()
          } else if (val > 44000 && cash_target != 0.17) {
            cash_target = 0.17
            setTargets()
          } else if (val < 38000 && cash_target != 0.15) {
            cash_target = 0.15
            setTargets()
          } else if (val < 35000 && cash_target != 0.14) {
            cash_target = 0.14
            setTargets()
            buy_only = true
          }
        ...
        // useCurrentAlloc: set targets to the current allocation
        function setTargets(useCurrentAlloc=false) {
          var targets = target_prop.slice();
          if (useCurrentAlloc) {
            checkAll(false, function(account_value, all_balances) {
        ...
        function rebalance(rebalance_now) {
          ...
          for (var i=0; i<ss.length; i++) {
            ...
            if (target_value > summary.asset_value * (1+trigger_pct)) {
              ...
              if (size > 0.01) {
                console.log("Buy ", summary.product ,"with", n(target_value-summary.asset_value).format('0.00'), "size:", n(size).format('0.000'), "for", Math.round(summary.target_pct*1000)/1000)
                //size = 0.01
              ...
              if (size > 0.01) {
                console.log("Sell ", summary.product ,"with", n(summary.asset_value - target_value).format('0.00'), "size:", n(size).format('0.000'), "for", Math.round(summary.target_pct*1000)/1000)
                //size = 0.010                                                                   
