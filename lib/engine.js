...
module.exports = function container (get, set, clear) {
  ...
  return function (s) {
      ...
      s.exchange.getBalance(s, function (err, balance) {
        ...
        if (!s.start_capital) {
          s.exchange.getQuote({product_id: s.product_id}, function (err, quote) {
            ...
            //temp = (n(s.balance.asset).add(n(s.balance.asset_hold))).multiply(quote.ask).value()
            //console.log("\nTotal value of " + s.product_id + ": " + temp)
            cb()
          })
        }
        else cb()
    ...
    placeOrder = function  (type, opts, cb) {
      if (!s[type + '_order']) {
        s[type + '_order'] = {
          price: opts.price,
          size: opts.size,
      ...
      if (so.mode !== 'live') {
        ...
        s.exchange[type](order, function (err, api_order) {
          if (err) {
            //console.log("-------------------error in placeorder function")
    ...
    // places a simple Limit order, or market order
    // if post only, will adjust to the appropriate price if rejected
    function placeLimitOrder (type, opts, cb) {
      if (!s[type + '_order']) {
        s[type + '_order'] = {
          price: opts.price,
          size: opts.size,
      ...
      var order = s[type + '_order']
      order.price = opts.price
      order.size = n(opts.size).format('0.00000000')
      if (typeof opts.type !== 'undefined') order.type = opts.type
      if (typeof opts.post_only !== 'undefined') {
        order.post_only = opts.post_only
      } else {
        if (order.type == 'limit') order.post_only = true
      }
      if (order.type == 'market') order.post_only = false
      ...
      if (so.mode !== 'live') {
        ...
        s.exchange[type](order, function order_cb(err, api_order) {
          if (err) {
            //console.log("-------------------error in placeorder function")
            console.log(err.body)
            return cb(err)
          ...
          if (api_order.status === 'rejected') {
            if (api_order.reject_reason === 'post only') {
              ...
              getQuote(function (err, quote) {
                  ...
                  if (type == "buy") {
                    opts.price = n(quote.bid).value()
                  } else {
                    opts.price = n(quote.ask).value()
                  }
                  console.log("rejected, post only. Reordering at market, type:", type)
                  return placeLimitOrder(type, opts, cb)
              })
              return(null, false)
            }
            else if (api_order.reject_reason === 'balance') {
              // treat as a no-op.
              console.log("rejected, balance")
              return cb(null, false)
            }
            var err = new Error('order rejected')
            err.order = api_order
          ...
          //console.log("success ===", type, order.price)
          return cb(null, order)
    ...
    function executeSignal (signal, _cb, size) {
      ...
      syncBalance(function (err) {
        ...
        // no need to wait if there is enough currency
        if (signal === 'buy' && Number(s.balance.currency_hold) > 10000000) {
        ...
        else if (signal === 'sell' && Number(s.balance.asset_hold) > 10000000) {
        ...
        getQuote(function (err, quote) {
          ...
          if (signal === 'buy') {
            ...
            if (so.markup_pct<0) {
              price = n(quote.ask).subtract(0.01).format(String(s.product.increment))
            } else if (n(price).add(0.01)<n(quote.ask) && n(so.markup_pct)==0 && n(currentPrice) != n(price)) {
              price = n(price).add(0.01).format(String(s.product.increment))
              console.log("outbidding at:", price)
            }
            currentPrice = price
            // set size
            if (!size) {
              if (s.options.max_val != 'undefined') {
                size = n(s.options.max_val).multiply(so.buy_pct).divide(100).divide(price).format('0.00000000')
              } else {
                size = n(s.balance.currency).multiply(so.buy_pct).divide(100).divide(price).format('0.00000000')
              }
            }
            if (Number(size) >= s.product.min_size) {
              if (s.product.max_size && Number(size) > s.product.max_size) {
          ...
          // SELL ORDER
            ...
            if (so.markup_pct<0) {
              price = n(quote.bid).add(0.01).format(String(s.product.increment))
            } else if (n(price).subtract(0.01)>n(quote.bid) && n(so.markup_pct)==0 && n(currentPrice) != n(price)) {
              price = n(price).subtract(0.01).format(String(s.product.increment))
              console.log("outbidding at:", price)
            }
            currentPrice = price
            // set size
            if (!size) {
              temp = s.balance.asset
              if (s.options.max_val != 'undefined') {
                if ( s.product.max_size > s.balance.asset) temp = s.options.max_val
              } 
              size = n(temp).multiply(so.sell_pct / 100).format('0.00000000')
            }
            if (Number(size) >= s.product.min_size) {
              if (s.product.max_size && Number(size) > s.product.max_size) {
                size = s.product.max_size
      ...
      function doOrder (retries=5) {
        placeOrder(signal, {
          ...
          if (err) {
            ...
            if (err.body && err.body.message == 'Insufficient funds') {
              console.log("====retrying====")
              // waiting for settlement from cancelled order
              if (retries>0) {
                return setTimeout(function() {doOrder(retries-1)}, 1000)
              } else {
                return cb(err)
              }
            }
        ...
        if (so.mode === 'live' || trade.price <= Number(s.buy_order.price)) {
          ...
          if (so.mode !== 'live') {
            ...
            if (s.exchange.makerFee) {
              fee = total.multiply(s.exchange.makerFee / 100).value()
              var currency_minus = n(s.balance.currency).subtract(fee).format('0.00000000')
              if (n(currency_minus).value() >= 0) {
                s.balance.currency = currency_minus
              }
              else {
                s.balance.asset = n(s.balance.asset).subtract(n(fee).divide(price)).format('0.00000000')
              }
            }
          }
          s.action = 'bought'
    ...
    return {
      ...
      getOrder: function (opts, cb) {
        s.exchange.getOrder(opts, function (err, order) {
          if (err) return cb(err)
          cb(null, order)
        })
      },
      ...
      getBook: function (cb) {
        s.exchange.getBook({product_id: s.product_id}, function (err, book) {
          if (err) return cb(err)
          cb(null, book)
        })
      },
      
      get5minstats: function (cb) {
        this.getXminstats(5, cb)
      },
      
      getXminstats: function (x, cb) {
        s.exchange.getXminstats({product_id: s.product_id, minutes:x}, function (err, stats) {
          if (err) return cb(err)
          cb(null, stats)
        })
      },
      ...
      getDayStats: function (cb) {
        s.exchange.get24hourstats({product_id: s.product_id}, function (err, stats) {
          if (err) return cb(err)
          cb(null, stats)
        })
      },
